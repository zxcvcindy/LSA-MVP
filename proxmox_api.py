...
def create_user_vm(
    ...
    try:
        # ① Clone
        r = requests.post(
            f"{BASE_URL}/nodes/{node}/qemu/{template_vmid}/clone",
            headers=HEADERS,
            verify=VERIFY_SSL,
            data={"newid": new_vmid, "name": vm_name, "full": 1},
        )
        r.raise_for_status()
        ...
        r = requests.post(
            f"{BASE_URL}/nodes/{node}/qemu/{new_vmid}/config",
            headers=HEADERS,
            verify=VERIFY_SSL,
            data={"ciuser": username, "cipassword": password, "ipconfig0": ipconfig},
        )
        r.raise_for_status()

        # ③ Start VM
        r = requests.post(
            f"{BASE_URL}/nodes/{node}/qemu/{new_vmid}/status/start",
            headers=HEADERS,
            verify=VERIFY_SSL,
        )
        r.raise_for_status()
        ...
        try:
            requests.delete(
                f"{BASE_URL}/nodes/{node}/qemu/{new_vmid}",
                headers=HEADERS,
                verify=VERIFY_SSL,
            ).raise_for_status()
        except Exception:
...
def restart_vm(node, vmid):
    ...
    if shutdown_resp.status_code != 200:
        return {"error": "Failed to shutdown VM", "details": shutdown_resp.text}
